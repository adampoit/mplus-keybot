...
{
	public BlizzardProfileClient(HttpClient client, ILogger<BlizzardProfileClient>? logger = null)
	{
	...
	public async Task<IReadOnlyList<VerifiedCharacter>> GetProfileCharactersAsync(string accessToken, string region, CancellationToken cancellationToken = default)
	{
		...
		using var request = new HttpRequestMessage(HttpMethod.Get, $"https://{normalizedRegion}.api.blizzard.com/profile/user/wow?namespace=profile-{normalizedRegion}&locale=en_US");
